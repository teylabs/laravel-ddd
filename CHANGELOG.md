...
- Prevent duplicate listener/subscriber registration across repeated package registration calls. Subscriber handlers are now registered through `subscribe()` rather than also being independently discovered as listeners.
- Apply `ddd.autoload_ignore`, custom autoload filters and overridden finders to listener and subscriber discovery. Preserve the application's event-discovery callback after scanning.
- Honor `ddd.autoload.migrations=false` even when migration paths are already cached.
- Correct request class references generated by `ddd:controller --requests`, including nested controller names, absolute references and custom request namespaces.
- Preserve custom configuration keys, layers and explicit lists during configuration sync. Missing package-defined map options receive defaults while explicit consumer values are retained.
- Make configuration saves repeatable without mutating namespace values. Preserve unknown top-level keys, literal values and existing symlink targets; check staged writes and replacement failures, clean up temporary files, and retain existing POSIX permission bits. Formatting failures fall back to the rendered configuration.

### Changed
- Refine generator execution and cleanup around Laravel's command lifecycle, consolidate namespace and child-command resolution, and separate internal discovery from registration.
...
- Isolate test applications from installed Testbench files, simplify repetitive generator datasets, and add a pinned consumer-behavior comparison harness.

### Upgrade notes
- PHP and Laravel dependency requirements are unchanged. This release does not introduce `tey/mod`.
- Rebuild discovery caches with `php artisan ddd:optimize` during deployment and restart long-running workers. Newly added objects and changed event types still require a cache rebuild. Composer's `--classmap-authoritative` mode requires an autoload rebuild to load renamed replacements; moving files without renaming their classes may also require rebuilding Composer's map.
- Ensure subscriber handlers are registered by their `subscribe()` method. Rebuild older listener manifests to apply discovery filters.
- Configuration sync now preserves explicit lists and the consumer-owned `layers` map, including empty values, instead of merging lists by numeric position. Empty package option maps such as `autoload` and `namespaces` still receive missing defaults.
- Saving configuration through a dangling symlink now fails without modifying the link, rather than creating its missing target.
