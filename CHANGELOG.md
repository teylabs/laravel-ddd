...
- If you cache discovery in production, run `php artisan ddd:optimize` after updating so listener filters take effect. Restart any long-running workers to load the updated code.
- If you use event subscribers, ensure their `subscribe()` method registers every handler you want to run. Subscriber methods are no longer also registered independently by discovery.
